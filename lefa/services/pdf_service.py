    ...
    def generar(factura: Factura) -> Path:
        ...
        if factura.numero_factura and factura.fecha_emision:
            ...
            try:
                ...
                pdf.image(str(qr_tmp), x=165, y=8, w=32)
                pdf.set_xy(163, 42)
                pdf.set_font("Helvetica", size=6)
                pdf.multi_cell(36, 3, "QR tributario", align="C")
            finally:
                if qr_tmp.is_file():
        ...
        if prefs.emisor_telefono.strip():
            pdf.cell(0, 5, f"Tel: {prefs.emisor_telefono.strip()}", ln=True)
        pdf.cell(0, 5, prefs.emisor_email, ln=True)
        if prefs.emisor_iban.strip():
            pdf.cell(0, 5, f"IBAN: {prefs.emisor_iban.strip()}", ln=True)
        pdf.ln(8)
